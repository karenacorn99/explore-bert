...
class SwagExample(object):
    ...
    ''' string representation of each training/testing example '''
    def __str__(self):
...
''' Bert representation for each training/testing example'''
...
# read training/testing examples from file
def read_swag_examples(input_file, is_training):
    ...
    examples = [
        SwagExample(
            swag_id = line[2], # fold-ind
            context_sentence = line[4], # sent1
            start_ending = line[5], # sent2, in the swag dataset, the
...
''' Each training example is trained as 4 binary classification task '''
''' The model outputs a single value for each pair of context and ending '''
''' The final decision is based on a softmax over all values '''
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    # features is a list of InputFeatures objects
    return features

''' truncate input to max_length in place by removing the last element from the longer array '''
...
''' compute accuracy of prediction(out/logits) given correct labels(labels) '''
def accuracy(out, labels):
    outputs = np.argmax(out, axis=1)
    return np.sum(outputs == labels)

'''choices_features is a list of 4 dictionaries of 
  'input_ids': input_ids,
  'input_mask': input_mask,
  'segment_ids': segment_ids '''
